defmodule AdamJournal.Admin.AgentsLive do
  use Sigil.Live
  import Sigil.HTML, only: [escape: 1]

  @impl true
  def mount(params, socket) do
    agents = AdamJournal.Agents.list_agents()

    {agent, editing, show_form} =
      case params do
        %{"id" => id} -> {AdamJournal.Agents.get_agent!(id), true, true}
        _ -> {nil, nil, false}
      end
  ...
  def handle_event("save_agent", params, socket) do
    ...
    result =
      if socket.assigns.editing do
        AdamJournal.Agents.update_agent(socket.assigns.selected_agent, attrs)
      else
        AdamJournal.Agents.create_agent(attrs)
      end
  ...
  def handle_event("delete_agent", _params, socket) do
    if socket.assigns.selected_agent, do: AdamJournal.Agents.delete_agent(socket.assigns.selected_agent)
    agents = AdamJournal.Agents.list_agents()
  ...
  defp render_tool_checkboxes(selected_tools) do
    AdamJournal.ToolRegistry.all_with_info()
    |> Enum.sort_by(& &1.slug)
    |> Enum.map_join("\n", fn tool ->
