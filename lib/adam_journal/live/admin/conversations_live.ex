defmodule AdamJournal.Admin.ConversationsLive do
  use Sigil.Live
  import Sigil.HTML, only: [escape: 1]

  @impl true
  def mount(params, socket) do
    conversations = AdamJournal.Conversations.list_conversations(limit: 50)
    ...
    if selected_id, do: AdamJournal.ConversationPubSub.subscribe(selected_id)
  ...
  def handle_event("admin_send", %{"message" => message}, socket) when message != "" do
    ...
    AdamJournal.Conversations.add_message(conv_id, "admin", message)
    AdamJournal.ConversationPubSub.broadcast(conv_id, {:new_message, %{role: "admin", content: message}})
  ...
  defp load_conversation(id) do
    conv = AdamJournal.Conversations.get_conversation!(id)
