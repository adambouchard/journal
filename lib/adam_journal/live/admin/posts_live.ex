defmodule AdamJournal.Admin.PostsLive do
  use Sigil.Live
  import Sigil.HTML, only: [escape: 1, escape_attr: 1]

  @impl true
  def mount(params, socket) do
    posts = AdamJournal.Blog.list_posts()

    {post, editing, show_form} =
      case params do
        %{"id" => id} -> {AdamJournal.Blog.get_post!(id), true, true}
        _ -> {nil, nil, false}
      end
  ...
  def handle_event("auto_save", params, socket) do
    ...
    result =
      if socket.assigns.editing do
        AdamJournal.Blog.update_post(socket.assigns.selected_post, attrs)
      else
        AdamJournal.Blog.create_post(attrs)
      end
  ...
  def handle_event("delete_post", _params, socket) do
    if socket.assigns.selected_post, do: AdamJournal.Blog.delete_post(socket.assigns.selected_post)
    posts = AdamJournal.Blog.list_posts()
  ...
  def handle_event("toggle_published", _params, socket) do
    ...
    if socket.assigns.selected_post do
      AdamJournal.Blog.update_post(socket.assigns.selected_post, %{published: new_published})
    end
