defmodule AdamJournal.HomeLive do
  use Sigil.Live
  ...
  def mount(params, socket) do
    ...
    posts =
      if active_tag && active_tag != "" do
        AdamJournal.Blog.list_published_posts_by_tag(active_tag)
      else
        AdamJournal.Blog.list_published_posts()
      end
      ...
      Enum.map_join(assigns.posts, "\n", fn post ->
        ...
        <article class="group">
          ...
          <h3 class="mt-3 font-serif text-3xl sm:text-4xl lg:text-5xl font-medium leading-[0.95] tracking-tight text-stone-900 dark:text-stone-100 group-hover:text-stone-600 dark:group-hover:text-stone-300 transition-colors">
            <a href="/entry/#{post.slug}">#{escape(post.title)}</a>
          </h3>
    ...
    <div class="flex min-h-screen flex-col lg:flex-row">
      ...
      <main class="flex-1 min-w-0 flex flex-col">
        <div class="flex-1 mx-auto max-w-3xl w-full px-6 py-8 lg:px-10 lg:py-12">
          #{if assigns[:active_tag] do
            """
            <div class="flex items-center gap-3 mb-8">
              <p class="text-xs font-semibold uppercase tracking-widest text-stone-500">Tagged: #{escape(assigns.active_tag)}</p>
              <a href="/" class="inline-flex items-center gap-1 rounded-full bg-stone-200 dark:bg-stone-800 px-2.5 py-0.5 text-xs font-medium text-stone-600 dark:text-stone-400 hover:bg-stone-300 dark:hover:bg-stone-700 transition-colors">
                Clear ✕
              </a>
            </div>
            """
