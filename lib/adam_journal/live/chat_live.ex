defmodule AdamJournal.ChatLive do
  @moduledoc """
  ...
  def mount(params, socket) do
    ...
    agent_config =
      case params do
        %{"slug" => slug} -> AdamJournal.Agents.get_agent_by_slug!(slug)
        _ -> List.first(AdamJournal.Agents.list_active_agents())
      end

    slug = if agent_config, do: agent_config.slug, else: "default"
    tags = AdamJournal.Blog.list_tags()
    recent_posts = AdamJournal.Blog.list_published_posts() |> Enum.take(3)

    # Load ALL active agent configs from DB — team is fully DB-driven
    agent_configs = AdamJournal.Agents.list_active_agents()
    ...
    {conversation, messages, team, session_writes} =
      if ai_available and socket.connected? do
        ...
        AdamJournal.ConversationPubSub.subscribe(conv.id)
  ...
  def render(assigns) do
    ...
    sidebar =
      AdamJournal.Components.SideNav.render(%{
        tags: assigns[:tags] || [],
  ...
  def handle_event("send_message", %{"message" => message}, socket) when message != "" do
    ...
    AdamJournal.Conversations.add_message(conv_id, "user", message)
    AdamJournal.ConversationPubSub.broadcast(conv_id, {:new_message, %{role: "user", content: message}})

    # Auto-title from first user message
    if length(socket.assigns.messages) <= 1 do
      case AdamJournal.Conversations.get_conversation!(conv_id) do
        conv -> AdamJournal.Conversations.maybe_set_title(conv, message)
      end
    end
  ...
  defp dispatch_and_send(socket, message) do
    ...
    Task.start(fn ->
      ...
      target_slug = AdamJournal.Dispatch.classify(message, recent_context, agent_configs)
  ...
  defp start_team(agent_configs, conversation) do
    ...
    api_key = Application.get_env(:sigil, :anthropic_api_key)
    team_name = :"adam_journal_#{conv_id || System.unique_integer([:positive])}"
      ...
      {:error, _} ->
        ...
        agents =
          Enum.map(agent_configs, fn config ->
            ...
            {String.to_atom(config.slug), AdamJournal.GenericAgent, opts}
          end)
  ...
  defp welcome_message(config), do: "Hi! I'm #{config.name}. Ask me anything about the blog."

  defp resume_or_create_conversation(existing_conv_id, agent_config) do
    conversation =
      if existing_conv_id do
        try do
          conv = AdamJournal.Conversations.get_conversation!(existing_conv_id)
    ...
    if conversation do
      ...
      {:ok, conv} =
        AdamJournal.Conversations.create_conversation(%{
