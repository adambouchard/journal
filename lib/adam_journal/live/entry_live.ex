defmodule AdamJournal.EntryLive do
  use Sigil.Live
  import Sigil.HTML, only: [escape: 1]

  @impl true
  def mount(params, socket) do
    post = AdamJournal.Blog.get_post_by_slug!(params["slug"])
    {prev_post, next_post} = AdamJournal.Blog.adjacent_posts(post)
    ...
    <div class="mx-auto max-w-3xl px-6 py-8 lg:px-8 lg:py-16">
      ...
      <header class="flex flex-col items-center gap-2 mb-10 pb-8 border-b border-stone-200 dark:border-stone-800">
        <a href="/" class="group flex flex-col items-center gap-2.5">
          <img src="https://avatars.githubusercontent.com/u/362348?v=4" alt="" class="size-10 rounded-full ring-2 ring-stone-200 dark:ring-stone-700 group-hover:ring-stone-400 dark:group-hover:ring-stone-500 transition-all" />
          <span class="font-serif text-lg font-medium tracking-tight text-stone-500 dark:text-stone-400 group-hover:text-stone-900 dark:group-hover:text-stone-100 transition-colors">#{escape(AdamJournal.Settings.get("site_name"))}</span>
        </a>
      </header>
  ...
  defp render_nav(prev, next) do
    prev_html =
      if prev do
        """
        <a href="/entry/#{prev.slug}" class="group flex items-center gap-3 max-w-[45%]">
    ...
    next_html =
      if next do
        """
        <a href="/entry/#{next.slug}" class="group flex items-center gap-3 max-w-[45%] ml-auto text-right">
          <div class="min-w-0">
