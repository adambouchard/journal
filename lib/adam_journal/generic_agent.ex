defmodule AdamJournal.GenericAgent do
  @moduledoc """
  ...
  def init_agent(opts) do
    ...
    tool_modules = AdamJournal.ToolRegistry.resolve(tool_slugs)
  ...
  defp blog_context do
    posts = AdamJournal.Blog.list_published_posts()
    ...
    if posts == [] do
      ...
      Below are the published entries from adam journal. Reference these naturally when relevant.
  ...
  def on_complete(response, state) do
    ...
    if conv_id do
      ...
      AdamJournal.Conversations.add_message(conv_id, "ai", content)
      AdamJournal.ConversationPubSub.broadcast(conv_id, {:new_message, %{role: "ai", content: content}})
    end
